...
def build_model(model_name, pretrained, device, jit=False):
    model, _, preprocess = open_clip.create_model_and_transforms(model_name, pretrained, device=device, jit=jit)
...
def infer_batch(model: open_clip.CLIP, input_images, input_promts):
    with torch.no_grad(), torch.cuda.amp.autocast():
        image_features = model.encode_image(input_images)
        text_features = model.encode_text(input_promts)
